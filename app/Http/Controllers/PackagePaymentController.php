...
class PackagePaymentController extends Controller
{
    ...
    public function aamarpay_package_payment_done($package_id, $mer_txnid)
    {
        ...
        $package_payment->payment_method = 'AamarPay';
        $package_payment->payment_details = 'Success';
        ...
        if ($package_payment->save()) {
            ...
            // Check if reference for this sale set or not in user table
                        // Calculate commision
            // Take commision amount
            // sales amount
            // calculate commition receovable amount*CommionAmount%
            // Save to referel user commion trans table
            // update sumery table

            $refer_user = Auth::user()->refer;
            $referrar_id = ReferralCode::where('code', $refer_user)->first();
