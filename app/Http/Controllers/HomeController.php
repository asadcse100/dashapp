...
class HomeController extends Controller
{
    ...
    {
        $data = [];
        $data['product_categories'] = DB::table('product_categories')->select('id', 'description', 'name', 'image')->where('status',1)->where('id', $id)->first();
        $data['product_services'] = DB::table('product_services')->select('id', 'title', 'short_description', 'image')->where('status',1)->where('category_id', $id)->get();

        return view('frontend.default.product_details', $data);
    }

    public function products_sub_details($id,$service_id)
    {
        $data = [];
        $data['product_categories'] = DB::table('product_categories')->select('id', 'description', 'name', 'image')->where('status',1)->where('id', $id)->first();
        $data['product_services'] = DB::table('product_services')->select('id', 'title', 'short_description', 'image')->where('status',1)->where('category_id', $id)->where('product_service_id',$service_id)->get();
    ...
    {
        $subCategory = DB::table('product_services')->where('category_id', $request->category_id)->get();
        return $subCategory;
    }
