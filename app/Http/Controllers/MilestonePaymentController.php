...
class MilestonePaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->payment_option == 'aamarpay'){
            ...
            return $aamarpay->aamarpay_payment_url();
        }
    ...
    public function aamarpay_milestone_payment_done($milestone_request_id, $mer_txnid)
    {
        $translations_log = Translations_log::where('mer_txnid', $mer_txnid)->first();

        $milestone_req = MilestonePayment::findOrFail($milestone_request_id);
        ...
        if ($milestone_req != null) {
            $milestone_req->payment_details = "Success";
            $milestone_req->payment_method = 'AamarPay';
            if ($package->commission_type == 'percent') {
                $milestone_req->admin_profit = ($translations_log->amount * $package->commission)/100;
            }
            else {
                $milestone_req->admin_profit = $package->commission;
            }
            $milestone_req->freelancer_profit = $translations_log->amount - $milestone_req->admin_profit;
        ...
        if(!empty($refer_user)){
            ...
            $ref_commision_amount = $ref_commision*$translations_log->amount/100;
            ...
            DB::table('transactions')->updateOrInsert(
                ...
                [
                    ...
                    'trans_total' => $translations_log->amount,
                    'method' => 'AamarPay',
                    'status' => 2
                ]
            );
