...
class VerificationController extends Controller
{
    ...
    public function verification_store(Request $request)
    {
        ...
        if(!empty($request->verification_file)){
            $imageName = time().'.'.$request->verification_file->extension();                    // Public Folder
            $request->verification_file->move(public_path('profile/verification_file'), $imageName);
            $verification->attachment = $imageName;
        }
