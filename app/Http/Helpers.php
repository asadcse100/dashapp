...
function referral($type='', $prams=[])
{
    ...
    // $user = Auth::user();
    $data['total'] = User::where('refer', 1)->count();
    // $chart = [];
    ...
    return view('misc.panel.card-referral', $data)->render();
}
...
if (!function_exists('the_uid')) {
    ...
    {
        return str_pad($userID, 5, '0', STR_PAD_LEFT);
    }
}
...
if (!function_exists('expert_user')) {

    function expert_user($category_id = null, $subcategory_id = null)
    {
        $data = [];
        if(!empty($category_id) && !empty($subcategory_id) ){
            $data['cat_subcat'] = DB::select('select * from users u
            ...
            $data['cat'] = DB::select('select * from users u
...
if (!function_exists('custom_asset')) {
    function custom_asset($id)
    {
        if (\App\Upload::find($id) != null) {
            return asset(\App\Upload::find($id)->file_name);
        }
        return null;
    }
}
