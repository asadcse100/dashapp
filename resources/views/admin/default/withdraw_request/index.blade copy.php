                    ...
                    <div class="card-header row gutters-5">
                        ...
                        <div class="col-md-3 ml-auto">
                            <select class="form-control aiz-selectpicker mb-2 mb-md-0" name="type" id="type"
                                onchange="sort_withdraw_request_list()">
                        ...
                        <option value="requested_amount,desc"
                            @isset($col_name, $query) @if ($col_name == 'requested_amount' && $query == 'desc') selected @endif
                        @endisset>{{ translate('Sort by Request Amount (High > Low)') }}</option>
                    <option value="requested_amount,asc"
                        @isset($col_name, $query) @if ($col_name == 'requested_amount' && $query == 'asc') selected @endif
                    @endisset>{{ translate('Sort by Request Amount (Low > High)') }}</option>
            </select>
        </div>
        <div class="col-md-3">
                ...
                <tr>
                    ...
                    <td class="text-right">
                        ...
                        <a class="btn btn-soft-primary btn-icon btn-circle btn-sm btn icon"
                            href="{{ route('pay_to_freelancer', encrypt($withdraw_request->id)) }}"
                            title="{{ translate('Pay Now') }}">
                            <i class="las la-money-check"></i>
                        </a>
                        <a href="{{ route('pay_to_freelancer.cancel', encrypt($withdraw_request->id)) }}"
